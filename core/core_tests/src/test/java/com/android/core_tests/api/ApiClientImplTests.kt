package com.android.core_tests.api

import com.android.api.ApiClient
import com.android.api_service.ApiService
import com.android.core_model.AuthDto
import com.android.core_model.LoginRequest
import com.android.core_model.ProductDto
import com.android.core_model.UpdateProduct
import com.android.core_tests.CoroutineTestRule
import com.android.core_tests.runTest
import com.android.impl.ApiClientImpl
import io.mockk.MockKAnnotations
import io.mockk.clearAllMocks
import io.mockk.coEvery
import io.mockk.coVerify
import io.mockk.mockk
import org.junit.After
import org.junit.Before
import org.junit.Rule
import org.junit.Test
import retrofit2.Response

class ApiClientImplTest {

    @get:Rule
    val coroutineRule = CoroutineTestRule()

    private lateinit var apiService: ApiService
    private lateinit var apiClient: ApiClient

    @Before
    fun setup() {
        MockKAnnotations.init(this, relaxUnitFun = true)
        apiService = mockk()
        apiClient = ApiClientImpl(apiService)
    }

    @After
    fun tearDown() {
        coroutineRule.cancelScopeAndDispatcher()
        clearAllMocks()
    }



    @Test
    fun `retrieveProducts should call apiService and return its response`() =
        coroutineRule.runTest {
            // Given
            val expected = listOf(
                ProductDto(1, "Title", 1f, "Desc", "Cat", "img", null)
            )
            val response = Response.success(expected)
            coEvery { apiService.retrieveProducts() } returns response

            // When
            val result = apiClient.retrieveProducts()

            // Then
            coVerify(exactly = 1) { apiService.retrieveProducts() }
            assert(result === response)
        }

    @Test
    fun `retrieveSingleProduct should call apiService with id and return its response`() =
        coroutineRule.runTest {
            // Given
            val id = 42
            val dto = ProductDto(id, "Title", 2f, "Desc2", "Cat2", "img2", null)
            val response = Response.success(dto)
            coEvery { apiService.retrieveSingleProduct(id) } returns response

            // When
            val result = apiClient.retrieveSingleProduct(id)

            // Then
            coVerify(exactly = 1) { apiService.retrieveSingleProduct(id) }
            assert(result === response)
        }

    @Test
    fun `userLogin should call apiService with request and return its response`() =
        coroutineRule.runTest {
            // Given
            val request = LoginRequest("user", "pass")
            val dto = AuthDto("token123")
            val response = Response.success(dto)
            coEvery { apiService.userLogin(request) } returns response

            // When
            val result = apiClient.userLogin(request)

            // Then
            coVerify(exactly = 1) { apiService.userLogin(request) }
            assert(result === response)
        }

    @Test
    fun `updateProduct should call apiService with id and update and return its response`() =
        coroutineRule.runTest {
            // Given
            val id = 7
            val update = UpdateProduct(
                id = id,
                title = "New",
                price = 3.14f,
                description = "d",
                category = "c",
                image = "i"
            )
            val dto = ProductDto(
                id,
                update.title,
                update.price,
                update.description ?: "",
                update.category ?: "",
                update.image ?: "",
                null
            )
            val response = Response.success(dto)
            coEvery { apiService.updateProduct(id, update) } returns response

            // When
            val result = apiClient.updateProduct(id, update)

            // Then

            coVerify(exactly = 1) { apiService.updateProduct(id, update) }
            assert(result === response)
        }
}
